class Api::V1::TodaySerializer < Api::V1::BaseSerializer
	...
	class BoxSerializer < Api::V1::BaseSerializer
		...
		def orders
			object.orders.where(["status = '1' and date(start_time) = ? and time(start_time) > ?",Date.today,Time.now.to_time]).map do |order|
		...
		def offorders
			object.offorders.where("status = 't' and date(start_time) = ? and datetime(start_time) > ?",Date.today,DateTime.now).map do |offorder|
